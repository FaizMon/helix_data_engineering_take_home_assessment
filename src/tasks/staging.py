...
def transform_stg_loans(conn):
    ...
    for row in rows:
        ...
        try:
            ...
            logger.warning(f"Rejecting loan {loan_id}: {e}")
            rejected += 1
            continue
    ...
    logger.info(f"stg_loans: {raw_count} in, {inserted} out, {rejected} rejected")
    return {"rows_in": raw_count, "rows_out": inserted, "rows_rejected": rejected}


def transform_stg_payments(conn):
    ...
    logger.info(f"stg_payments: {raw_count} in, {inserted} out, {rejected} rejected")
    return {"rows_in": raw_count, "rows_out": inserted, "rows_rejected": rejected}
